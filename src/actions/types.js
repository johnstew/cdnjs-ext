export const SEARCH_STARTED = 'SEARCH_STARTED';
export const SEARCH_DONE = 'SEARCH_DONE';
export const SEARCH_FAIL = 'SEARCH_FAIL';

export const GET_DATA_SOURCE_DONE = 'GET_DATA_SOURCE_DONE';
export const GET_DATA_SOURCE_FAIL = 'GET_DATA_SOURCE_FAIL';
export const GET_DATA_SOURCE_STARTED = 'GET_DATA_SOURCE_STARTED';

export const SELECT = 'SELECT';
export const VERSION_CHANGE = 'VERSION_CHANGE';
export const VERSION_CHANGE_DONE = 'VERSION_CHANGE_DONE';

export const INCREMENT_PAGE_INDEX = 'INCREMENT_PAGE_INDEX';
